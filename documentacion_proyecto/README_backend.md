...
**Casos de uso:**
- **Pantalla 21 del formulario**: Muestra únicamente programas relevantes al usuario basados en sus selecciones previas.
...
- `programa`: **CAMPO NUEVO Y MEJORADO** - Nombre completo del programa específico seleccionado por el usuario en la pantalla 21. Se obtiene dinámicamente mediante filtrado inteligente basado en las selecciones previas de profesión, formación y modalidad. Si no hay programas disponibles para la combinación seleccionada, este campo se envía vacío y se muestra un mensaje personalizado al usuario.
- Campos tipo array (`reasons`, `learning_methods`, etc.) se almacenan como JSON en la base de datos

**Flujo avanzado de selección de programa:**
1. Usuario selecciona profesión (pantalla 3), formación (pantalla 4) y modalidad (pantalla 12)
2. Al navegar a pantalla 21: se ejecuta automáticamente la carga dinámica de programas via `get_programas_filtrados.php`
3. **Filtrado inteligente**: Solo se muestran programas que coinciden exactamente con las tres selecciones previas
4. **Manejo de casos sin resultados**: Si no hay programas disponibles, se muestra mensaje personalizado: "No tenemos disponible un programa relacionado a {Formación} en tu {Línea} en la {Modalidad} para ti, estaremos en contacto."
5. **Validación condicional**: El botón enviar se habilita cuando se selecciona un programa o cuando no hay programas disponibles
6. **Almacenamiento optimizado**: Se guarda el nombre completo del programa (no el ID) para facilitar la lectura y análisis posterior
