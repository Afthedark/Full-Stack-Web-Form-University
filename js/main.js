...
toStep4Btn.addEventListener('click', () => {
  ...
  if (modalityCheckboxes.length && toStep13Btn) {
    modalityCheckboxes.forEach(checkbox => {
      checkbox.addEventListener('change', () => {
        ...
        // Guardar ID de modalidad seleccionada
        if (selectedModality) {
          window.selectedModalidadId = selectedModality;
          localStorage.setItem('selectedModalidadId', selectedModality);
          console.log('Modalidad seleccionada - ID:', selectedModality);
        }
    ...
    submitFormBtn.addEventListener('click', async (e) => {
      ...
      if (!submitFormBtn.disabled) {
        const formData = {
          ...
          programa: '', // Campo vacío - el programa se seleccionará en pantalla 23
          contact: contactData
        };
        ...
        try {
          ...
          if (result.success) {
            // Navegar a la pantalla de carga (step-22 = índice 21)
            goToStep(21);
            
            console.log('✓ Navegación a pantalla 22 completada');
            console.log('✓ Iniciando timeout de 8 segundos...');
            
            // Simular carga durante 8 segundos y luego mostrar botón
            setTimeout(() => {
              console.log('✓✓✓ TIMEOUT EJECUTADO - 8 SEGUNDOS PASADOS ✓✓✓');
              
              // Buscar dentro de la pantalla 22 activa
              const step22 = document.getElementById('step-22');
              console.log('step-22 encontrado:', step22);
              
              if (step22) {
                const toStep23Btn = step22.querySelector('#toStep23');
                
                console.log('toStep23Btn:', toStep23Btn);
                
                // Solo mostrar el botón, el GIF se mantiene visible
                if (toStep23Btn) {
                  toStep23Btn.classList.add('button-visible');
                  console.log('✓ Botón mostrado - Clase button-visible agregada');
                }
              } else {
                console.log('ERROR: No se encontró step-22');
              }
              
              console.log('✓ Carga completada - Botón visible, GIF sigue mostrándose');
            }, 8000);
          } else {
  ...
  function goToStep(stepIndex) {
    ...
    // Si llegamos a la pantalla 23 (índice 22), cargar los programas
    if (stepIndex === 22) {
      console.log('✓ Llegamos a pantalla 23 - Cargando programas...');
      if (typeof obtenerYMostrarProgramas === 'function') {
        obtenerYMostrarProgramas();
      } else {
        console.error('Error: función obtenerYMostrarProgramas no está disponible');
      }
    }
    ...
    const hideProgressScreens = [0, 6, 9, 12, 19]; // 0-based: 1,7,10,13,20 (22 y 23 MUESTRAN la barra)
    if (mainContainer) {
    ...
    if (mainContainer && headerContainer && footer && body) {
      const specialBackgroundScreens = [1, 2, 3, 4, 5, 7, 8, 10, 11, 13, 14, 16, 17, 18, 19, 20, 21, 22]; // Incluir pantallas 22 y 23 (índices 21 y 22)
  ...
  // Pantalla 22: Mostrar botón después de 8 segundos
  const toStep23Btn = document.getElementById('toStep23');
  if (toStep23Btn) {
    toStep23Btn.addEventListener('click', () => {
      goToStep(22); // Ir a pantalla 23 (índice 22)
    });
  }
  ...
  window.addEventListener('resize', () => {
    const headerContainer = document.querySelector('.header-container');
    const specialBackgroundScreens = [1, 2, 3, 4, 5, 7, 8, 10, 11, 13, 14, 16, 17, 18, 19, 20, 21, 22]; // Incluir pantalla 22 y 23
  ...
  const initialSpecialScreens = [1, 2, 3, 4, 5, 7, 8, 10, 11, 13, 14, 16, 17, 18, 19, 20, 21, 22]; // Incluir pantalla 22 y 23
