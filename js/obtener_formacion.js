    ...
    formaciones.forEach(formacion => {
        ...
        optionSpan.addEventListener('click', function() {
            if (!optionSpan.classList.contains('selected')) {
                ...
                // Guardar ID de formación seleccionada
                window.selectedFormacionId = formacion.id;
                localStorage.setItem('selectedFormacionId', formacion.id);
                console.log('Formación seleccionada - ID:', formacion.id, 'Nombre:', formacion.nivel_formacion);
            }
